...
@export var playerSceneRoot: Node
@export var LaunchSFX: AudioStream
...
func _process(_deltaTime):
	...
	if (weight.weight > 1)&&Input.is_action_just_pressed("Launch"):
		...
		playerSceneRoot.add_child(projectileInstance)
		AudioOneshot.playAt(LaunchSFX, global_position)
